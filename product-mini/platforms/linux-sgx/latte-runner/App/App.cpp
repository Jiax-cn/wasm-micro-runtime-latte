...
main(int argc, char *argv[])
{
    ...
    /* Load WASM byte buffer from reserved wasm section */
    // if (SGX_SUCCESS != ecall_read_wasm_sec(g_eid, &wasm_file_buf, &wasm_file_size)) {
    //     printf("Call ecall_read_wasm_sec() failed.\n");
    //     goto fail1;
    // }
    ...
    BENCHMARK_STOP(collect);
    printf("collect time :     %3ld s %10ld ns\n", collect.tv_sec , collect.tv_nsec);
#if WASM_ENABLE_STATIC_PGO != 0
    if (gen_prof_file)
